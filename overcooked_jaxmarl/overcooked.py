...
BASE_REW_SHAPING_PARAMS = {
    ...
    "PLATE_PICKUP_REWARD": 4, # reward for picking up a plate when soup-making is active
    "SOUP_PICKUP_REWARD": 8, # reward for picking up a ready soup
    "DELIVERY_COMPLETION_REWARD": 6, # shaped bonus on successful soup delivery
    "SOUP_DROP_PENALTY": 1, # penalty for dropping a held soup/dish before delivery
...
# Pot status indicated by an integer, which ranges from 8 to 0
# 8 = empty, 7 = 1 onion, 6 = 2 onions, 5 = 3 onions (full, starts cooking), 4..1 cooking, 0 ready.
POT_COOK_TIME = 5
MAX_ONIONS_IN_POT = 3 # A pot has at most 3 onions. A soup contains exactly 3 onions.
POT_FULL_STATUS = POT_COOK_TIME
...
class Overcooked(MultiAgentEnv):
    ...
    def step_env(
        ...
        state, (reward_0, reward_1), shaped_rewards, debug_events = self.step_agents(key, state, acts)
        ...
        return (
            ...
            dones,
            {
                ...
                'debug_events': {
                    'onion_in_pot': debug_events[0],
                    'soup_pickup': debug_events[1],
                    'delivery': debug_events[2],
                    'missed_ready': debug_events[3],
                },
            },
        )
    ...
    def reset(
        ...
        key, subkey = jax.random.split(key)
        # Pot status is determined by a number between 0 (inclusive) and POT_EMPTY_STATUS (inclusive)
        # POT_EMPTY_STATUS corresponds to an empty pot (default)
    ...
    def step_agents(
    ...
    ) -> Tuple[State, Tuple[float, float], Tuple[float, float], chex.Array]:

        # Ready-pot diagnostics before applying actions.
        height = self.obs_shape[1]
        padding = (state.maze_map.shape[0] - height) // 2
        pot_x_dbg = state.pot_pos[:, 0]
        pot_y_dbg = state.pot_pos[:, 1]
        pot_status_dbg = state.maze_map.at[padding + pot_y_dbg, padding + pot_x_dbg, 2].get()
        any_pot_ready_before = jnp.any(pot_status_dbg == POT_READY_STATUS)
        ...
        candidate_maze_map, alice_inv, alice_reward, alice_shaped_reward, alice_events = self.process_interact(maze_map, state.wall_map, fwd_pos, state.agent_inv, 0)
        ...
        alice_events = jax.lax.select(alice_interact, alice_events, jnp.zeros((3,), dtype=jnp.float32))

        candidate_maze_map, bob_inv, bob_reward, bob_shaped_reward, bob_events = self.process_interact(maze_map, state.wall_map, fwd_pos, state.agent_inv, 1)
        ...
        bob_events = jax.lax.select(bob_interact, bob_events, jnp.zeros((3,), dtype=jnp.float32))

        onion_in_pot = alice_events[0] + bob_events[0]
        soup_pickup = alice_events[1] + bob_events[1]
        delivery = alice_events[2] + bob_events[2]
        missed_ready = jnp.array(any_pot_ready_before, dtype=jnp.float32) * (1.0 - jnp.minimum(soup_pickup, 1.0))
        debug_events = jnp.array([onion_in_pot, soup_pickup, delivery, missed_ready], dtype=jnp.float32)
        ...
        if self.cooperative:
            reward_0 = alice_reward + bob_reward
            reward_1 = alice_reward + bob_reward
        else:
            reward_0 = alice_reward
            reward_1 = bob_reward
    ...
    def process_interact(
        ...
        shaped_reward += has_picked_up_plate * is_dish_picku_useful * BASE_REW_SHAPING_PARAMS["PLATE_PICKUP_REWARD"]

        # Extra dense signal on the final interact that completes delivery.
        shaped_reward += successful_delivery * BASE_REW_SHAPING_PARAMS["DELIVERY_COMPLETION_REWARD"]

        # Penalize dropping cooked soup (dish) on counters instead of delivering it.
        dropped_dish = successful_drop * holding_dish
        shaped_reward -= dropped_dish * BASE_REW_SHAPING_PARAMS["SOUP_DROP_PENALTY"]
        ...
        debug_events = jnp.array([
            jnp.array(case_1, dtype=jnp.float32),
            jnp.array(case_2, dtype=jnp.float32),
            jnp.array(successful_delivery, dtype=jnp.float32),
        ], dtype=jnp.float32)
        return maze_map, inventory, reward, shaped_reward, debug_events
