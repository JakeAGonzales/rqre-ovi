...
def qre_jax(
    ...
    """Batched QRE solver. Q1, Q2: [B, A, A]. Returns pi1, pi2, V1, V2."""
    B = Q1.shape[0]
    A = num_actions
    ...
    def body_fn(carry, _):
        ...
        def continue_iteration(args):
            pi1_c, pi2_c, _ = args
            u1 = jnp.einsum('bij,bj->bi', Q1, pi2_c)
            u2 = jnp.einsum('bi,bij->bj', pi1_c, Q2)
...
def qre_single(
    ...
    A = Q1.shape[0]
    pi1_init = jnp.ones(A, dtype=jnp.float32) / A
    pi2_init = jnp.ones(A, dtype=jnp.float32) / A
...
class QREStagHunt:
    """Finite-horizon QRE-OVI for Stag Hunt."""
    ...
    def select_action(self, state: State, h: int) -> Tuple[int, int]:
        ...
        pi1, pi2, _, _ = qre_single(
            jnp.array(Q1), jnp.array(Q2), self.eps1, self.eps2,
            max_iter=self.qre_max_iter)
...
def save_agent(agent: QREStagHunt, path: str) -> None:
    checkpoint = {
        ...
        "eps1": agent.eps1, "eps2": agent.eps2, "qre_max_iter": agent.qre_max_iter,
...
def load_agent(path: str) -> QREStagHunt:
    ...
    agent = QREStagHunt(
        ...
        qre_max_iter=ck.get("qre_max_iter", 100),
